...
<script>
  ...
  export default{
    data(){
      return {
        urlsAr: JSON.parse(localStorage.getItem("cachePhotoWater")) || "",
        loadProgress: "00",
        radius: 150,
        maxPhoto: 150,
        isComplate: false,
    ...
    computed: {
      arrayImages() {
        ...
        /*      filterNoActive(){
         return this.arrayImages.filter((item, ind) => {
         if (item.show === false &&
         ((item.leftpoz < this.objEvent.epx + this.radius && item.leftpoz > this.objEvent.epx - this.radius)
         && (item.toppoz < this.objEvent.epy + this.radius && item.toppoz > this.objEvent.epy - this.radius) )) {
         if (this.arrayImages[ind].id == item.id) this.arrayImages[ind].show = true;
         ...
         allActive(){
         return this.arrayImages.filter((item) => {
         return item.show === false;
         }).length;
         }*/
      complated(){
    ...
    mounted(){
      localStorage.setItem("cachePhotoWater", JSON.stringify(this.urlsAr));
    }
  }
</script>
