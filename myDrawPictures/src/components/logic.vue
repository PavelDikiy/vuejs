<template>
    <div id="container" @mousemove="mouseMoveEv" :class="{'complete': isComplate}">
        ...
        <img v-for="(imgd) in arrayImages" :key="imgd.id" :src="imgd.imgurl" :width="imgd.width"
...
<script>
  ...
  export default{
    props: ['storage', 'urlsAr', 'title', 'radius', 'nextStep'],
    data(){
      return {
        loadProgress: "00",
        isComplate: false,
    ...
    computed: {
        ...
        for (let i = 0; i < this.urlsAr.length; i++) {
          ...
          if (!localStorage.getItem(this.storage)) {
            setTimeout(() => {
              ...
              img.onload = () => {
                this.loadProgress = Math.ceil((count * 100) / this.urlsAr.length);
      ...
      complated(){
        ...
        for (let i = 0; i < this.arrayImages.length; i++) {
          ...
          if (i === this.arrayImages.length - 1) {
            setTimeout(() => {
